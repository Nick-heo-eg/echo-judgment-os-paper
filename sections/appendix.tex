\appendix
\section{Supplementary Material}\label{app:appendix}
\subsection{Additional Pseudo-Code}
\begin{lstlisting}[language=Python]
def trace_signature_generate(mode, resonance_vector, permanence):
    payload = {
        "mode": mode,
        "resonance": resonance_vector,
        "permanence": permanence,
        "hash": deterministic_hash(payload_without_hash),
        "timestamp": now_iso()
    }
    return payload
\end{lstlisting}

\subsection{SRL Stability Sketch}
Assuming $f_{\mathrm{align}}$ is a contraction mapping with coefficient $\beta < 1$, the SRL update $r_{t+1} = (1-\alpha)r_t + \alpha f_{\mathrm{align}}(r_t)$ inherits contraction with coefficient $(1-\alpha)+\alpha\beta < 1$. Banach's fixed-point theorem ensures convergence.

\subsection{Benchmark Execution Plan for Gemini}
Gemini benchmark runs are scheduled for the next iteration. The protocol mirrors the GPT/Claude evaluation: (1) SRL drift measurement under adversarial prompts, (2) override rate calculation during strategic compliance tests, (3) judgment consistency across model swaps, (4) latency distribution under Echo OS Decision Kernel with varying context sizes. Placeholder values in Tables 1--2 will be replaced upon completion of the full experimental suite. The unified architecture ensures that Gemini integration requires no modifications to the OS layer itself.

\subsection{Trace Signature Example}
\begin{verbatim}
---
Trace Signature
{mode: ECHO_JUDGEMENT, resonance: [0.71, -0.02, 0.33],
 rhythm: steady, clarity: 0.94, permanence: true,
 hash: 9f3c0af1, timestamp: 2025-12-11T10:22:00Z}
\end{verbatim}

\subsection{Proof Capsule Snippet}
\begin{verbatim}
proof_id: proof-20251211-001
context:
  request_id: req-789
  semantic_frame: RESONANCE_GUIDE
judgement:
  lookup_version: v3.7
  validation_status: accepted
\end{verbatim}
